...
alert("Welcome to Brian\'s Space Battle page ! - open the console log on the following page to play . Hurry the fate of the planet is at your fingertips Captian! If you chose to retreat at any time just leave the page. But know that choice has doomed Earth! ")
...
const battleWinCheck = (badGuy) => { if (badGuy.hull <= 0){
    console.log("You blew up " + badGuy.name +" Nice job!");
    victory();
} else {console.log("keep shooting nice attack keep playing")};  
};


/// check if game is over -------******** if yes alert oh joy --- else ... if not over have next alien ship in [ bad guys ] attack you

const didIWin = () => { if (badGuys[0] === "dead"){ // this is working now
...
alert("Awesome job playing you won !!!! Close this and refresh to play again"); 
} else {
console.log('%c ... There are still aliens left to kill ...','background:red; color:white;');
console.log('%c .. Look out here comes another one now ..','background: lightyellow; color:red; font-size:15px;');
if (badGuys[2] === "dead" ){
badGuys[0].attack(youHero); 
} else { badGuys[0].fight(youHero); /// #5 and #6 will use more powerful mizzles to attack you - better use reddit to fix your hull captian they have unlimited shots upgraded hull fix to 2 times so can win  ......... 
}
}};
...
const victory = () => { if (badGuys[0].hull <= 0) {
console.log('%c !! Awesome !!.. you destroyed the alien ship ..!!','background:orange; font-size:15px; border:3px solid green;');
...
if (youHero.hull <=0){
    ...
    alert(` Oh No your ship blew up ! The ${youHero.name} is gone! Close this annoying pop-up and refresh the page to try again Captian! Good thing it was just a simulation..... don't forget your engineers can search Reddit for a way to restore your hull power`)
} else { console.log('%c ..... Your turn now captian enter your move below ..... ','background:lightskyblue;' );}
};
...
class Hero {
    constructor (name, hull, accuracy) {
        ...
        this.catchPhrases = ['Get to the chopper!', 
        'It\'s not a tumor!', `Consider that a divorce!`, `This ship is the Governator we are going to bring the pain!`]
        }
        talkSas() {
            let rUp = this.catchPhrases[Math.floor(Math.random()*this.catchPhrases.length)];
        ...
        useReddit(){
            if (enigneers[0] === "E" ){
                ...
                console.log(this.name + "we found the answer on Reddit! We fixed our shield generator hull strength improved" );
                this.hull+=10;
                this.announceHealth();
                console.log('%c It is still your turn','background: lightskyblue;')
            } else {
                console.log('%c Your engineers are all on a coffe break - sorry Captian they can\'t do it.... they do not have the power!', 'background:lightskyblue;');
            }}
        attack (badGuy) {if (this.hull >=1) { //////attack with missles if you are still alive
            if(badGuy.name === badGuys[0].name){ //// attack the right bad guy
             if (Math.floor(Math.random()*Math.floor(9))/10<= this.accuracy ){
        //// above random to determine if hit or not roughly 80% sucess rate
                if (myMissles[0] == "M"){ ////// can only use certian amount of missles
                    console.log(this.talkSas());
                    console.log(this.name + " used  missles 10 hitpoints dealt");
                    console.log(badGuy.name + " got hit with a missle, their health is down to");
                ...
                console.log('%c awwww .... raaaattttssssssssssss...... you are out of missles switch to lasers','background:lightskyblue;' );
                ///// does not count as turn - could be typo
                console.log('%c ..... it is still your turn now captian enter your move below ..... ','background:lightskyblue;' );
            } }else { 
                console.log('%c Awww hamburgers... You missed','background:lightgray; color:red;');
                victory(badGuy); //// you missed but that was your turn they get to shoot at you now
            } } else {console.log('%c What are you shooting at ? dead aliens or the wrong target - choose another target or refresh the page to play again','background:black; color:white; font-size:20px;');
                console.log('%c ..... Your turn now captian enter your move below ..... ','background:lightskyblue;' ); //// if you target a dead alien or .... does not count as turn - could be typo
    
        }} else{ 
            console.log('you are dead refresh the game to play again') ///// if your hull is less than 1 and you are trying to attack still
        }}
        laser (badGuy) {if (this.hull >=1) { //////attack with lasers if you are still alive ... /// ... following logic similiar to missle but no array to access for shot count since lasers are unlimited currently.... 
            if(badGuy.name === badGuys[0].name){
                if (Math.floor(Math.random()*Math.floor(9))/10<= this.accuracy ){
                    /// above random to determine if hit or not roughly 80% sucess rate
                console.log(this.talkSas());
                 console.log(this.name + " used laser " + this.weapons.lasers + " hitpoints dealt");
                 console.log(badGuy.name + " got hit with S \-\ lasers, their health is down to");
                 console.log(badGuy.hull +=-this.weapons.lasers);
                 victory(badGuy);
            } else {
            console.log('%c Oh dang it dang it dang it.... that shot was close but unfortunately you missed ','background:lightskyblue;');
            victory(badGuy);
            }} else{
                console.log('%c You are shooting dead aliens - choose another target or refresh the page to play again','background:black; color:white; font-size:20px;');
                console.log('%c ..... it is still your turn now captian enter your move below ..... ','background:lightskyblue;'); 
             }} else{ 
        console.log('you are dead refresh the game to play again'); ///// if your hull is less than 1 and you are trying to attack still
    }}};
...
class Alienship {
    constructor (name) {
        ...
        this.catchPhrases = ['Puny humans you are no match for us!', 
        '- Feel our alien wrath! ', `- How deep do you want to get probed huumoonnn? `, `- You are no match for our superior alien technology and intellect! `, `- Garp grrizzrr fluffn halftry schmoop! - what you thought they only spoke English? `, `- KLAATU BARADA NIKTO - what you thught that was just a movie? `, `- Not bad, for a huumooon ... now taste our wrath!`, `- Hope you like the taste of lazzzzerszzzssss huumoonnn...`, `- Jerry charge the mizzles and tell those dum huumons we come in peace so they let us just kill them !`, `- Put some butter and jelly on yourself huumon. You are now Toast ! `]
    }    
        talkSmack() {
            let rIp = this.catchPhrases[Math.floor(Math.random()*this.catchPhrases.length)];
            console.log(this.name + " says " + rIp);
        }
        announceHealth(){
            console.log("We are the " + this.name + " our shields are now " + this.hull);
        }
        fight(youHero) { if (this.name === badGuys[0].name){
            if ((Math.floor(Math.random()*Math.floor(9))) /10 <= this.accuracy){
            ...
            console.log('%c We got hit by alien lazers with a z  our hull power is down to ... ','background:lightskyblue; ');
            ...
            console.log("good those alien scumbags missed us " + youHero.name + "took no damage. Hull power at " +youHero.hull); /// you know they missed
...
///////// mizzles for the "boss" or last 2 enemies  
        attack(youHero) { if (this.name === badGuys[0].name){
            if ((Math.floor(Math.random()*Math.floor(9))) /10 <= this.accuracy){
            console.log(this.talkSmack());
            console.log(this.name + " used  mizzlez with a z dealing 7 hit points to you ship..");
            console.log(youHero.name + " got hit with an alien Z \* mizzle, their health is down to");
            ...
            console.log("Ha! those aliens shoot like stormtroopers " + youHero.name + "took no damage. Hull power at" +youHero.hull); /// you know they missed
...
//// will build top line first then each subsequent pushed before it into array - construct in reverse order because i want ship 1 to be first target .... it's my game.... 
...
console.log(' %c........ Welcome to the Space Battle! You are captian of the USS Schwarzenegger - and there are 6 evil alien ships headed to destroy Earth! ........ ', 'font-size: 15px; background:black; border: 2px solid red; color:white;' )
...
console.log("......The future has some bright spots also your amazing piolt will dodge some attacks and the the aliens targeting computer will only make around 50% of their shots..... ")
console.log("......Remember if you are getting whooped you can have your engineers search Reddit for a way to fix your hull but only a limited amount of times...... ")
console.log("......your ship is below check it out before you get started cilck the arrow...... ")
console.log(youHero);
console.log("......your ship" +youHero.name +  "has just left the moons orbit on its way to Kiber 7 for a ping pong tournament when...... ")
...
console.log("......You can see the scanner results above by clicking on the little arrow beside the (6) [Alienship, Alienship, ....] on the above line or a few lines up depending on your screen size it's right under the yellow beep warning...... ")
